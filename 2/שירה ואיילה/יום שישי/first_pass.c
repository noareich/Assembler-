...
void handle_extern_directive(AssemblerState* state, const char* label) {
   ...
   if (state->extern_count == state->extern_capacity) {
    ...
    state->extern_table[state->extern_count].addresses = NULL;  /* Initialize addresses to NULL */
state->extern_table[state->extern_count].addresses[0] = 0;  // Initialize first address to 0
    state->extern_count++;
}
...
void print_extern_table(AssemblerState* state) {
    ...
    printf("Name\tAddress\n");
    for (int i = 0; i < state->extern_count; i++) {
printf("%s\t%04d\n", state->extern_table[i].name, state->extern_table[i].addresses[0]);
    }
}
/*
