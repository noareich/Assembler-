...
typedef struct {
    ...
    int entry_count;
    int entry_capacity;
    ExternLabel* extern_table;  /* Changed from char** to ExternLabel* */
    int extern_count;
    int extern_capacity;
    int IC;
    int DC;
...
void handle_extern_directive(AssemblerState* state, const char* label) {
   ...
   if (state->extern_count == state->extern_capacity) {
    ...
    state->extern_table[state->extern_count].address = 0;  /* Initialize address to 0 */
    state->extern_count++;
}
...
void print_extern_table(AssemblerState* state) {
    ...
    printf("Name\tAddress\n");
    for (int i = 0; i < state->extern_count; i++) {
        printf("%s\t%04d\n", state->extern_table[i].name, state->extern_table[i].address);
    }
}
